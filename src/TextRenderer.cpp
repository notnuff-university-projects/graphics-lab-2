...
bool TextRenderer::Init() {
    ...
    textShader_ = Shader("./rsc/shaders/gl_textVertexShader.glsl", "./rsc/shaders/gl_textFragmentShader.glsl");
...
void TextRenderer::RenderText(std::string text,
    ...
    // Activate textShader_ and pass the time uniform
    glm::mat4 projection = glm::ortho(0.0f, static_cast<float>(WIN_WIDTH), 0.0f, static_cast<float>(WIN_HEIGHT));
    glUniformMatrix4fv(glGetUniformLocation(textShader_.ID, "projection"), 1, GL_FALSE, glm::value_ptr(projection));

    textShader_.use();
    float timeValue = glfwGetTime() * 3;
    glUniform1f(glGetUniformLocation(textShader_.ID, "time"), timeValue);
    glUniform1f(glGetUniformLocation(textShader_.ID, "scale"), 0.01f);
